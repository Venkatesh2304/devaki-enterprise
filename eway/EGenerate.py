...
def EGenerate(types,date1,date2,data) :
   if types == "Both" : 
      ...
      emain("eway",date1,date2,False,data)
   elif  types == "EINVOICE" : 
        emain("einvoice",date1,date2,False,data)
   else :
        emain("eway",date1,date2,False,data)
...
def download_e(etype,date1,date2,driver,data={},beats = [] ,bills = []) : 
 ...
 if etype == "einvoice" : 
   type_data = {"Doc Column": "Document Number" , "fileinfo" : {"title":"E Invoice,E Invoice","reportfilename":"E_Invoice",
     ...
     type_data = {"Doc Column": "Document Number" , "fileinfo" : {"title":"E Way Bill,E Way Bill","reportfilename":"E_Way_Bill",
 ...
 if "ALL" in beats : #to make all possible  
     beats = [""]
     bills = [] 
     _data = {}
     _data["."] = data[".ALL"]
